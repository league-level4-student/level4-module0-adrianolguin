...
public class WorldPanel extends JPanel implements MouseListener, ActionListener {
	...
	public void mousePressed(MouseEvent e) {
		...
		int xPos = (int) Math.floor(e.getX() / cellSize);
		int yPos = (int) Math.floor(e.getY() / cellSize);

		if (xPos >= cells.length || yPos >= cells[0].length) {
			return;
		}

		if (cells[xPos][yPos].isAlive) {
			cells[xPos][yPos].isAlive = false;
		} else {
			cells[xPos][yPos].isAlive = true;
		}
